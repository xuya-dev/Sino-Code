...
function tempRoot(): string {
  const root = mkdtempSync(join(tmpdir(), 'sino-code-packaging-'))
  tempRoots.push(root)
  return root
}
...
describe('electron-builder Dragon packaging', () => {
  it('includes Dragon runtime dependencies in the packaged app', () => {
  ...
  it('validates the unpacked Dragon runtime before release artifacts are created', () => {
    const root = tempRoot()
    const context = createMacPackContext(root)
    const unpackedRoot = afterPack._internals.unpackedAppRoot(context)

    for (const relativePath of afterPack.DRAGON_RUNTIME_REQUIRED_PATHS) {
      touch(join(unpackedRoot, relativePath))
    }
    touch(join(unpackedRoot, 'node_modules/better-sqlite3/package.json'))

    expect(() => afterPack._internals.validateBundledDragonRuntime(context)).not.toThrow()

    rmSync(join(unpackedRoot, 'dragon/node_modules/zod'), { recursive: true, force: true })

    expect(() => afterPack._internals.validateBundledDragonRuntime(context)).toThrow(
  ...
  it('checks timestamp candidates across nested macOS signed code', () => {
    const root = tempRoot()
    const appBundle = join(root, 'Sino Code.app')
    const mainExecutable = join(appBundle, 'Contents/MacOS/Sino Code')
