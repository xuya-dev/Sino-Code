...
Sino Code（思诺智码）是一个面向国产大模型的本地桌面工作台，基于 DeepSeek GUI 项目二开。我们将原本仅支持 DeepSeek 的智能体工作台，扩展为**支持所有国产大模型**的通用桌面客户端，内置 DeepSeek、智谱 AI、MiniMax、月之暗面（Kimi）、阿里云百炼、腾讯（TokenHub / Coding Plan）、小米 7 个厂商预设，并提供 Coding Plan、按量、Token Plan 等多种端点供自由选择。
...
委派。`Dragon run` / `Dragon chat` / `Dragon exec` 可脱离 GUI 运行；GUI 的设置页
...
- 普通用户：可在设置页检查 GUI 更新，或前往 [GitHub Releases](https://github.com/xuyadev/Sino-Code/releases) 下载最新安装包。
...
| 文档 | 内容 |
| --- | --- |
| [docs/Dragon-architecture.md](docs/Dragon-architecture.md) | Dragon 单运行时方案、GUI 拆改范围、HTTP/SSE 合约、旧 agent 拆除说明 |
...
本项目是基于 [DeepSeek GUI](https://github.com/XingYu-Zhong/DeepSeek-GUI) 进行二次开发的，感谢原作者 [@XingYu-Zhong](https://github.com/XingYu-Zhong) 及其贡献者们打下的坚实基础。如果上游项目对你也有帮助，请前往 <https://github.com/XingYu-Zhong/DeepSeek-GUI> 给原作者一个 Star ⭐。
