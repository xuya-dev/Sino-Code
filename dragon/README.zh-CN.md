...
Dragon 是 Sino-Code 的本地 HTTP/SSE 代理运行时。它为桌面应用提供稳定、类型化且应用友好的代理循环合约：
...
- `DEEPSEEK_API_KEY` – 适配器转发到上游模型提供商的 API key，应用默认运行时需要
...
`{data-dir}/config.json`（若存在）。应用默认路径是：
...
`GET /v1/runtime/tools` 查看 provider 诊断。应用设置页会读取这两条接口。
...
JSONL 为追加式，即使包含部分格式错误行也可通过下一次重放跳过。应用可通过 `index.json` 列表并重放各线程 JSONL 来重建会话。
...
如需跨线程长期记忆，请用应用 Memory 面板创建 `memory_create` 工具记录。
